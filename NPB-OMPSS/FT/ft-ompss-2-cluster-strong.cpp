...
static void fft_calc(dcomplex u[]);
...
int main(int argc, char **argv){
	...
	/** @todo: uncomment here when able to fix the problem with u[] matrix */
	// fft_init(MAXDIM);
	fft(1, u1, u0);
	/*
	 * ---------------------------------------------------------------------
...
static void cfftz(int is,
	...
	/**
	 * @note: calculate u[] matrix here each time in a task.
...
static void compute_indexmap(void* pointer_twiddle,
	...
	for(k=0; k<d3; k+=BSIZE){
		...
		#pragma oss task out(twiddle[beg:end-1][0;d2][0;d1]) \
				private(k, kk, kk2, jj, kj2, ii, j, i) \
				firstprivate(ap, d2, d1, nzz, nyy, nxx, beg, end)
		for(k=beg; k<end; k++){
			kk = ((k+nzz/2) % nzz) - nzz/2;
			kk2 = kk*kk;
			for(j=0; j<d2; j++){
				jj = ((j+nyy/2) % nyy) - nyy/2;
				kj2 = jj*jj+kk2;
				for(i=0; i<d1; i++){
					ii = ((i+nxx/2) % nxx) - nxx/2;
...
static void compute_initial_conditions(void* pointer_u0,
	...
	for(k=0; k<d3; k+=BSIZE){
		...
		#pragma oss task in(starts[0;nzz]) out(u0[beg:end-1][0;d2][0;d1]) \
				private(k, j, x0) firstprivate(d2, nxx, beg, end)
		for(k=beg; k<end; k++){
			x0 = starts[k];
			for(j=0; j<d2; j++){
...
static void evolve(void* pointer_u0,
	...
	for(k=0; k<d3; k+=BSIZE){
		...
		#pragma oss task in(twiddle[beg:end-1][0;d2][0;d1]) \
				...
				private(k, j, i) firstprivate(d2, d1, beg, end)
		for(k=beg; k<end; k++){
			for(j=0; j<d2; j++){
				for(i=0; i<d1; i++){
...
static void fft(int dir,
	...
	if(dir==1){
		cffts1(1, dims[0], dims[1], dims[2], pointer_x1, pointer_x1,
				/*(dcomplex(*)[FFTBLOCKPAD])(void*)y1*/  NULL,
				/*(dcomplex(*)[FFTBLOCKPAD])(void*)y2)*/ NULL);
...
static inline void fft_calc(dcomplex u[]){
	int m,nu,ku,i,j,ln;
	double t, ti;
	/*
	 * ---------------------------------------------------------------------
